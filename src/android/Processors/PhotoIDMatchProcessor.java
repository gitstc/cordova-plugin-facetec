...
public class PhotoIDMatchProcessor extends Processor implements FaceTecFaceScanProcessor, FaceTecIDScanProcessor {
    ...
    private PhotoIDMatchProcessorCallback callback;
    private String latestExternalDatabaseRefID;

    private String TAG = "FacetecTest_Firas";

    public PhotoIDMatchProcessor(String sessionToken, Context context, String latestExternalDatabaseRefID, PhotoIDMatchProcessorCallback callback) {
        this.callback = callback;
        this.latestExternalDatabaseRefID = latestExternalDatabaseRefID;
        
        Log.d(TAG, "Latest External Database Ref ID: " + latestExternalDatabaseRefID);
        ...
        //
        Log.d(TAG, "Part 1:  Starting the FaceTec Session");
    ...
    public void processSessionWhileFaceTecSDKWaits(final FaceTecSessionResult sessionResult, final FaceTecFaceScanResultCallback faceScanResultCallback) {
        Log.d(TAG, "processSessionWhileFaceTecSDKWaits...");
        //
        // DEVELOPER NOTE:  These properties are for demonstration purposes only so the Sample App can get information about what is happening in the processor.
        // In the code in your own App, you can pass around signals, flags, intermediates, and results however you would like.
        //
        if(callback != null) {
            callback.onSessionResult(sessionResult);
        }
        ...
        if(sessionResult.getStatus() != FaceTecSessionStatus.SESSION_COMPLETED_SUCCESSFULLY) {
            Log.d(TAG, "sessionResult.getStatus() != FaceTecSessionStatus.SESSION_COMPLETED_SUCCESSFULLY");
            Log.d(TAG, "returning...");
        ...
        try {
            ...
            e.printStackTrace();
            Log.d(TAG, "Exception raised while attempting to create JSON payload for upload.");
        }
        ...
        NetworkingHelpers.getApiClient().newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(@NonNull Call call, @NonNull okhttp3.Response response) throws IOException {
                ...
                try {
                    ...
                    Log.d(TAG, "enrollment-3d - was Processed " + wasProcessed);
                    if(wasProcessed) {
                        ...
                        Log.d(TAG, "faceScanResultCallback.proceedToNextStep");
                        ...
                        Log.d(TAG, "faceScanResultCallback.cancel()");
                        faceScanResultCallback.cancel();
                    }
                }
                catch(JSONException e) {
    ...
    public void processIDScanWhileFaceTecSDKWaits(final FaceTecIDScanResult idScanResult, final FaceTecIDScanResultCallback idScanResultCallback) {
        Log.d(TAG, "processIDScanWhileFaceTecSDKWaits...");
        //
        // DEVELOPER NOTE:  These properties are for demonstration purposes only so the Sample App can get information about what is happening in the processor.
        // In the code in your own App, you can pass around signals, flags, intermediates, and results however you would like.
        //
        if(callback != null) {
            callback.onIDScanResult(idScanResult);
        }
        ...
        if(idScanResult.getStatus() != FaceTecIDScanStatus.SUCCESS) {
            Log.d(TAG, "idScanResult.getStatus() != FaceTecIDScanStatus.SUCCESS");
            Log.d(TAG, "returning...");
            NetworkingHelpers.cancelPendingRequests();
        ...
        NetworkingHelpers.getApiClient().newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(@NonNull Call call, @NonNull okhttp3.Response response) throws IOException {
                ...
                try {
                    ...
                    // In v9.2.0+, we key off a new property called wasProcessed to determine if we successfully processed the Session result on the Server.
                    // Device SDK UI flow is now driven by the proceedToNextStep function, which should receive the scanResultBlob from the Server SDK response.
                    Log.d(TAG, "match-3d-2d-idscan - wasProcessed " + wasProcessed);
                    if(wasProcessed) {
                        ...
                        Log.d(TAG, "idScanResultCallback.proceedToNextStep");
    ...
    public interface PhotoIDMatchProcessorCallback {
        void onSessionResult(FaceTecSessionResult result);
        void onIDScanResult(FaceTecIDScanResult result);
    }
}